...
The **CLI/TUI surface** (`src/quilt_hp/cli/`) changes when user-facing behaviour changes — command names, output formatting, interactive prompts. It is intentionally thin: `main.py` translates user intent into `QuiltClient` calls; it contains no business logic. This means you can use the library from any context (a Python script, a Home Assistant component, a FastAPI service) without pulling in CLI concerns.

The **`QuiltClient` façade** (`client.py`) changes when the public API surface changes — new high-level methods, new constructor parameters, new caching behaviour. It owns the authentication lifecycle, manages the single gRPC channel, and exposes convenient methods that hide proto message construction. Library consumers interact with this layer exclusively; they should never need to touch service classes or proto objects directly.

The **service layer** (`src/quilt_hp/services/`) changes when the gRPC contract changes — new RPC methods, changed request or response shapes. Each service class accepts a `grpc.aio.Channel`, constructs a stub, and translates between Python domain objects and proto messages. This translation is the only job of the service layer.

The **transport/auth layer** (`transport.py`, `auth.py`, `tokens.py`) changes when the authentication mechanism or connection behaviour changes — a new Cognito flow, different keepalive settings, a new metadata header. This layer is infrastructure: it knows nothing about HVAC, comfort settings, or schedules.
...
`cli/store.py` provides `FileStore`, the CLI's `TokenStore` implementation — the only place in the project that writes auth data to disk.
...
`QuiltClient` is the façade. It holds the gRPC channel (created lazily at first `login()` call), holds the current JWT as `self._token`, and implements `get_current_token()` — the callable the transport interceptor uses for metadata injection on every outbound call. It instantiates the three service classes against the channel and owns the snapshot TTL cache.
...
`tokens.py` defines the data types (`CachedTokens`, `TokenRefreshContext`) and protocols (`TokenStore`, `LegacyTokenStore`, `TokenRefreshHooks`, `TokenRefreshPolicy`). No business logic — pure contracts.
...
This constraint exists because the library's primary use cases — long-running daemons, Home Assistant integrations, TUI applications — are all inherently async. A synchronous wrapper that uses `asyncio.run()` internally would prevent callers from running the library inside an existing event loop. Providing a sync wrapper that spawns a new thread with its own loop creates race conditions around shared state (particularly the gRPC channel). The async-only design is simpler and more correct.
...
Quilt's mobile applications communicate with the cloud backend over gRPC. The wire protocol was reconstructed from the mobile apps and documented in `proto/cleaned/`. Using gRPC means the library speaks the same protocol as the official apps — the most stable and feature-complete interface available.
...
The gRPC channel, auth tokens, and cached snapshot all live on the `QuiltClient` instance. You can run multiple instances against different accounts or environments simultaneously without interference. This is not just a design goal — it is enforced by the structure: there are no module-level variables that hold connection or auth state.
